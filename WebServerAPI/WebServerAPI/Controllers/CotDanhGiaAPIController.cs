...
namespace WebServerAPI.Controllers
{
    public class CotDanhGiaAPIController : ApiController
    {
        ...
        /// <summary>
        /// Lấy kết quả đánh giá tổng hợp theo từng tháng trong năm
        /// </summary>
        /// <param name="_Year"></param>
        /// <returns></returns>
        [HttpGet]
        public IEnumerable<KetQuaDanhGia_Column_> GetKetQuaDanhGiaALL(int _Year)
        {
            ...
            for (int i = 1; i < 13; i++)
            {
                ...
                KetQuaDanhGiaMD.RHL = db.KETQUADANHGIAs.Where(p =>p.MUCDO == 1 &&
                                                                  p.TG >= start &&
                                                                  p.TG <= end)
                                                       .Count();
                KetQuaDanhGiaMD.HL = db.KETQUADANHGIAs.Where(p => p.MUCDO == 2 &&
                                                                  p.TG >= start &&   
                                                                  p.TG <= end)
                                                      .Count();
                KetQuaDanhGiaMD.BT = db.KETQUADANHGIAs.Where(p => p.MUCDO == 3 &&
                                                                  p.TG >= start &&
                                                                  p.TG <= end)
                                                      .Count();
                KetQuaDanhGiaMD.KHL = db.KETQUADANHGIAs.Where(p => p.MUCDO == 4 &&
                                                                   p.TG >= start &&
                                                                   p.TG <= end)
                                                       .Count();
        ...
        /// <returns></returns>
        [HttpGet]
        public IEnumerable<KetQuaDanhGia_Column_> GetKetQuaDanhGiaBPAll(int _MaBP, string _Year)
        {
            // Tạo khoảng thời gian
            int year = Convert.ToInt32(_Year);
